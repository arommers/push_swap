...
This tool gives us a visual representation of the steps and instructions taken by the radix sorting algorithm in real time. It is specifically noticable that our algorithm sorts by dividing our list into two "buckets" or Stacks by constantly pushing all 0 values into the right part, stack B, and then returning them to the left part, stack A, before repeating it for the next bit position.
